...
//////////////////////////////////////////////////////////////////////////
void vtCPathLine::execute(list<vtListTimeSeedTrace*>& listSeedTraces,
                          vector<int>& fromCells, vector<int>& toCells)
{
    listSeedTraces.clear();
    computePathLine(listSeedTraces, fromCells, toCells);
}
                                   ...
                                   vector<int>& fromCells, vector<int>& toCells)
{
    vtListParticleIter pIter = m_lSeeds.begin();
    for (; pIter != m_lSeeds.end(); pIter++)
        m_itsParticles.push_back(new vtParticleInfo(*pIter));

    // Trace until the end of the current time window, not FLT_MAX
    double finalTime = (double)m_pField->GetMaxTimeStep();

    m_traceActualStepCounts.clear();

    pIter = m_itsParticles.begin();
    int seedIdx = 0;
    for (; pIter != m_itsParticles.end(); pIter++, seedIdx++)
    {
        vtListTimeSeedTrace* trace = new vtListTimeSeedTrace;
        vtParticleInfo* thisParticle = *pIter;
        ...
        if (seedIdx < (int)fromCells.size())
            thisParticle->m_pointInfo.fromCell = fromCells[seedIdx];

        if (thisParticle->itsValidFlag == 1)
        {
            // advectParticle pushes directly to toCells at each exit point.
            advectParticle(m_integrationOrder, *thisParticle, currentT, finalTime, *trace, &toCells);
        }
        else
        {
            toCells.push_back(-1);  // invalid seed, keep in sync
        }

        m_traceActualStepCounts.push_back(m_lastFieldLineStepCount);
        // Always push one entry per seed so pl_list and toCells stay in sync.
        listSeedTraces.push_back(trace);
    }
}

...
{
	double norm = (double)sqrt(vec[0]*vec[0] + vec[1]*vec[1] + vec[2]*vec[2]);
	if(norm != 0.0)
	{
...
MATRIX4 rotate_matrix(int type, double angle) {
	...
	switch(type) {
		case 1: // rotate by x
			{
				double anglex = (double)(angle/180.0)*(double)3.14159;
		...
		case 2: // rotate by y
			{
				double angley = (double)(angle/180.0)*(double)3.14159;
		...
		case 3: // rotate by z
			{
				double anglez = (double)(angle/180.0)*(double)3.14159;
...
MATRIX3 rotate_matrix_axis(VECTOR3 &axis, double theta)
{
	...
	double one_m_cost = 1.0 - cost;
	double sint = sin(theta);
	double axisxy_one_m_cost = axis.x()*axis.y()*one_m_cost;
	double axisxz_one_m_cost = axis.x()*axis.z()*one_m_cost;
	double axisyz_one_m_cost = axis.y()*axis.z()*one_m_cost;
	m[0][0] = cost + axis.x()*axis.x()*one_m_cost;
	m[0][1] = axisxy_one_m_cost - axis.z()*sint;
	m[0][2] = axisxz_one_m_cost + axis.y()*sint;
	m[1][0] = axisxy_one_m_cost + axis.z()*sint;
	m[1][1] = cost + axis.y()*axis.y()*one_m_cost;
	m[1][2] = axisyz_one_m_cost - axis.x()*sint;
	m[2][0] = axisxz_one_m_cost - axis.y()*sint;
	m[2][1] = axisyz_one_m_cost + axis.x()*sint;
	m[2][2] = cost + axis.z()*axis.z()*one_m_cost;
	return m;
}

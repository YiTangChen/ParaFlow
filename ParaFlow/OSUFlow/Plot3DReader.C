...
VECTOR3** CPlot3DReader::ReadSolution(int& nodeNum)
{
	...
	double refval[4];
	double *pDensity, *pMomentumU, *pMomentumV, *pMomentumW;
	...
	try
	{
		...
		if(m_nTimevarying == 1)					// time-varying dataset
		{
			...
			fread(refval, sizeof(double), 4, fIn);
			numToRead = xdim;
			nodeNum = numToRead;
			fclose(fIn);

			pDensity = new double[numToRead];
			pMomentumU = new double[numToRead];
			pMomentumV = new double[numToRead];
			pMomentumW = new double[numToRead];
			...
			for(iFor = m_nFromTime; iFor <= m_nToTime; iFor++)
			{
				...
				// density, momentum
				fread(pDensity, sizeof(double), numToRead, fIn);
				fread(pMomentumU, sizeof(double), numToRead, fIn);
				fread(pMomentumV, sizeof(double), numToRead, fIn);
				fread(pMomentumW, sizeof(double), numToRead, fIn);
...
VECTOR3** CPlot3DReader::ReadSolution(int& nodeNum)
{
	...
	int iFor, xdim, ydim, zdim, numToRead;
	double refval[4];
	double *pDensity, *pMomentumU, *pMomentumV, *pMomentumW,*pEnergy;
	char *slFn = new char[_MAX_PATH];
	VECTOR3** pData;
	VECTOR3* pDataOneStep;
	...
	try
	{
		...
		// load in data
			...
			for(int iFor = 0; iFor < timesteps; iFor++)
			{
				...
				fread(pMomentumV, sizeof(double), numToRead, fIn);
				fread(pMomentumW, sizeof(double), numToRead, fIn);
				fread(pEnergy, sizeof(double), numToRead, fIn);
...
void CPlot3DReader::GetDataFromSln( VECTOR3* pData, 
									const double* pDensity, 
									const double* pMomentumU,
									const double* pMomentumV,
									const double* pMomentumW,
									const int size)
{
	int iFor;
									  ...
									  TVertex* pVertexTopo)
{
	...
	try
	{
		...
		fTemp = (double *)malloc(sizeof(double) * nodeNum);
		fread(fTemp, sizeof(double), nodeNum, fIn);					// X for nodes
		for(iFor = 0; iFor < nodeNum; iFor++)
			pVertexGeom[iFor].position[0] = fTemp[iFor];
		fread(fTemp, sizeof(double), nodeNum, fIn);					// Y for nodes
		for(iFor = 0; iFor < nodeNum; iFor++)
			pVertexGeom[iFor].position[1] = fTemp[iFor];
		fread(fTemp, sizeof(double), nodeNum, fIn);					// Z for nodes
		for(iFor = 0; iFor < nodeNum; iFor++)

...
int Solution::GetValue(int id, double t, VECTOR3& nodeData)
{
	...
	if (!isTimeVarying())
	{
		// steady field: ignore t, just use slot 0
		nodeData = m_pDataArray[0][id];
		return 1;
	}

	// Use real timestamps for interpolation when available.
	// timestamps[i] is the real time (seconds) of array slot i.
	if (!m_timestamps.empty())
	{
		// Out-of-range check against real time bounds
		if (t < m_timestamps.front() || t > m_timestamps.back())
			return -1;
		...
		double ratio = 0.0;
		if (highT < m_nTimeSteps)
		{
			double span = m_timestamps[highT] - m_timestamps[lowT];
			if (span > 0.0)
				ratio = (t - m_timestamps[lowT]) / span;
		}
		else
		{
			highT = lowT;  // clamp at last slot
		}
	...
	{
		// Fallback: no timestamps provided, treat t as fractional index (legacy behaviour)
		double adjusted_t = t - m_MinT;
		if (adjusted_t < 0.0 || adjusted_t > (double)(m_nTimeSteps - 1))
			return -1;

		int lowT = (int)floor(adjusted_t);
		double ratio = adjusted_t - floor(adjusted_t);
		int highT = lowT + 1;
		if (lowT >= m_nTimeSteps - 1) { highT = lowT; ratio = 0.0; }
...
void Solution::dumpData(string &prefix)
{
  string filename = prefix + "_vector.bin";
  ...
  outFile.write((char*)&m_nNodeNum, sizeof(int));
  // m_NodeNum = nLocalVertices * nVertLevels;
  outFile.write((char*)m_pDataArray[0], m_nNodeNum * sizeof(VECTOR3));

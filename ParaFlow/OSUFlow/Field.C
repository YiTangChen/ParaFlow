			   ...
			   double min_t)
{
	...
	m_MinT = min_t;  m_MaxT = 0.0;
}

CVectorField::CVectorField(Grid* pGrid, Solution* pSolution, Solution* vSolution, int timesteps, double min_t)
{
	assert((pGrid != NULL) && (pSolution != NULL));
	m_pGrid = pGrid;
	m_pSolution = pSolution;
...
CVectorField::CVectorField(Grid* pGrid, int timesteps, double min_t)
{
	assert(pGrid != NULL);
	m_pGrid = pGrid;
	m_nTimeSteps = timesteps;
...
CVectorField::~CVectorField()
{
	...
	if(m_vSolution != NULL) delete m_vSolution;
}
...
void CVectorField::dumpData(string &prefix)
{
	if (m_pGrid != NULL) {
		m_pGrid->dumpData(prefix);
	}
	string horizontal_prefix = prefix + "_Horizontal";
	if (m_pSolution != NULL) {
		m_pSolution->dumpData(horizontal_prefix);
	}
	string vertical_prefix = prefix + "_Vertical";
	if (m_vSolution != NULL) {
		m_vSolution->dumpData(vertical_prefix);
	}
}
						  ...
						  vector<VECTOR3>& vNodeData)
{
	...
	case T5_CELL:
		if(m_pGrid->getCellVertices(cellId, eCellTopoType, vVerIds) == -1) {
			std::cout << "grid get Cell vertices failed\n";
			return -1;
		}
...
int CVectorField::at_phys(VECTOR3 pos, double t, VECTOR3& vecData)
{
	...
	if(m_pGrid->GetCellType() == VORONOI) {
		...
		m_pGrid->interpolate(vecData, vNodeData, coeff);
	}
...
int CVectorField::at_phys_truelocal(VECTOR3 pos, double t, VECTOR3& vecData)
{
	vector<VECTOR3> vNodeData;
	PointInfo pInfo;
			  ...
			  const double t, VECTOR3& nodeData)
{
	...
	if(this->m_pGrid->GetCellType() == VORONOI) {
		...
		// interpolate in the cell
		m_pGrid->interpolate(nodeData, vNodeData, pInfo.MPASO_interpolant);
	}
	else {
		if(pos[2] == 1.0) {
		...
		double coeff[3];
		for(int i = 0; i < 3; i++) coeff[i] = pInfo.interpolant[i];
		m_pGrid->interpolate(nodeData, vNodeData, coeff);
	}
	return 1;
}
...
int CVectorField::at_phys(VECTOR3 pos, double t, VECTOR4& vecData)
{
	...
	vecData[0] = horizontal_vec[0];
	vecData[1] = horizontal_vec[1];
	vecData[2] = horizontal_vec[2];
	vecData[3] = vertical_vec[0];
			  ...
			  const double t, VECTOR4& nodeData)
{
	...
	// getCellVertices is called once; both solutions fetched in a single loop.
	constexpr int MAX_VERTS = 2 * MPASO_MAX_EDGES;
	int     vVerIds[MAX_VERTS];
	VECTOR3 horizontalData[MAX_VERTS];
	VECTOR3 verticalData[MAX_VERTS];

	MPASOGrid* mpGrid = static_cast<MPASOGrid*>(m_pGrid);
	int nVert = mpGrid->getCellVertices(pInfo.inCell, T5_CELL, vVerIds);
	if (nVert < 0) {
		std::cout << "grid getCellVertices failed\n";
		return -1;
	}
	...
	VECTOR3 horizontal_vec, vertical_vec;
	mpGrid->interpolate(horizontal_vec, horizontalData, nVert, pInfo.MPASO_interpolant);
	mpGrid->interpolate(vertical_vec,   verticalData,   nVert, pInfo.MPASO_interpolant);
	nodeData[0] = horizontal_vec[0];
	nodeData[1] = horizontal_vec[1];
	nodeData[2] = horizontal_vec[2];
	nodeData[3] = vertical_vec[0];
...
// volume of a cell
//////////////////////////////////////////////////////////////////////////
double CVectorField::volume_of_cell(int cellId)
{
...
void CVectorField::ScaleField(double scale) 
{
				  ...
				  VECTOR3& pos)
{
	...
	switch(eCellTopoType)
	{
		...
		case T3_CELL:	
			...
			m_pGrid->interpolate(pos, vData, coeff);
			break;
		default:
			break;
	}
...
void CVectorField::GetInflowRegion(vector<VECTOR3>& inflowVerts, const double t)
{
...
void CVectorField::GetOutflowRegion(vector<VECTOR3>& outflowVerts, const double t)
{
	VECTOR3 inwardFaceN[6];
	int iFor, jFor, kFor, xdim, ydim, zdim;
...
void CVectorField::GetTangentialflowRegion(vector<VECTOR3>& tanflowVerts, const double t)
{
	VECTOR3 inwardFaceN[6];
	int iFor, jFor, kFor, xdim, ydim, zdim;
...
// compute the curl
void CVectorField::curl(double deltaX, 
						double deltaY, 
						double deltaZ,		// grid spacing
...
{
	double x, y, z;
	double oneoverdeltax, oneoverdeltay, oneoverdeltaz;
...
{
	double xRatio, yRatio, zRatio;						// interpolation ratio
...
void CVectorField::GenerateLapField(int t, bool bToNormalize, VECTOR3* pLap)
{
	int iFor, jFor, kFor, index, xdim, ydim, zdim;
	VECTOR3 p1, p2, p3;
	double u, v, w, gridX, gridY, gridZ;

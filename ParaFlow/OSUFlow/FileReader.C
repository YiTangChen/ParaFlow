...
double* ReadStaticDataRaw(char* fname, int* dimension) 
{
  ...
  pData = new double[totalNum * 3];
  fread(pData, sizeof(double), totalNum*3, fIn);
  fclose(fIn);
  return(pData); 
}
			       ...
			       double* sMin, double* sMax) 
{
  ...
  for (int z = (int)sMin[2]; z<=(int)sMax[2]; z++) {
    for (int y = (int)sMin[1]; y<=(int)sMax[1]; y++) {
      long offset = (long)(z*dimension[0]*dimension[1]+y*dimension[0]+sMin[0])*3*4 + 12;
      fseek(fIn, offset, SEEK_SET); 
      int size = (int)(sMax[0]-sMin[0]+1)*3; 
      fread(p, sizeof(double), size, fIn); 
      p+=size; 
    }
  }
  fclose(fIn);
				     ...
				     int *dimension)
{
  ...
  for (int i=0; i<n_timesteps; i++) {
    ...
    totalNum = dimension[0] * dimension[1] * dimension[2];
    pData = new double[totalNum * 3];
    fread(pData, sizeof(double), totalNum*3, fVecIn);
    ppData[i] = pData; 
    fclose(fVecIn);
  }
  return(ppData); 
...
double** ReadTimeVaryingDataRaw(char *fname, int& n_timesteps, 
  ...
  ppData = new double*[numTimesteps]; 
  ...
  for(int iFor = 0; iFor < n_timesteps; iFor++) {
	...
	fread(p, sizeof(double), size, fVecIn);
...
double** ReadData(char *fname, double *dim, double *minB, 
  ...
  for(i = 0; i < (files == NULL ? n_timesteps : num_files); i++) {
    ...
    pData = new double[(int)(maxB[0] - minB[0] + 1) * 
		      (int)(maxB[1] - minB[1] + 1) *
	...
	subsize[nc_dim_ids[j]] = (MPI_Offset)(maxB[j] - minB[j] + 1);
      ...
      double *var_buf = new double[nflt];
      assert(var_buf != NULL);
	...
	if (strcmp(nc_var_names[j], "NULL") == 0) {
	  ...
	  assert(ncmpi_get_vara_double_all(fd, nc_var_id,
...
void Mpi_ioReadDataRaw(MPI_File f, double *dim, double *minB, 
  ...
  MPI_File_set_view(f, 0, MPI_double, filetype, (char *)"native",
		    MPI_INFO_NULL);
  assert(MPI_File_read_all(f, p, nflt,
			   MPI_double, &status) == MPI_SUCCESS);
  assert(status.count == (int)(sizeof(double)) * nflt);
  ...
  for (z = (int)minB[2]; z <= (int)maxB[2]; z++) {
    ...
    for (y = (int)minB[1]; y <= (int)maxB[1]; y++) {
      ...
      offset = (long)(z * dim[0] * dim[1] + y * dim[0] + minB[0]) *
		3 * sizeof(double) + headeroffset;
      size = (int)(maxB[0] - minB[0] + 1) * 3;
      fseek(f, offset, SEEK_SET);
      assert(fread(p, sizeof(double), size, f) > 0);

...
bool CurvilinearGrid::at_vertex(int verIdx, VECTOR3& pos)
{
	...
	*/
	double xpos = m_pVertex[verIdx].position[0];
	double ypos = m_pVertex[verIdx].position[1];
	double zpos = m_pVertex[verIdx].position[2];
	
	// pos.Set((double)xidx, (double)yidx, (double)zidx);
...
//////////////////////////////////////////////////////////////////////////
double CurvilinearGrid::cellVolume(int cellId)
{
	double volume;
	VECTOR3 c[8],cell;
...
int CurvilinearGrid::locate_close_vertex_cell(VECTOR3 pp, VECTOR3& v)
{
	...
	VECTOR3 c,initial_v;
	int closest;
	double min_distance = 3.402823466e+38F;
...
int	CurvilinearGrid::adaptive_vertex_cell_walk(VECTOR3& pp, VECTOR3& start_v,	VECTOR3* v)
{
	...
	double current_distance =	(pp[0]-c[0])*(pp[0]-c[0])+
		(pp[1]-c[1])*(pp[1]-c[1])+
		(pp[2]-c[2])*(pp[2]-c[2]);

	double stride_size = 0.1f;
	...
	for (;;) {
		...
		while (making_progress) 
		{
			...
			double post_step_distance;
			double best_post_step_distance = current_distance;
...
int	CurvilinearGrid::walk_on_x_boundary(VECTOR3 pp, int plane_pos,double& min_d2,VECTOR3& min_v)
{
	//dir:FEL_I;
...
int	CurvilinearGrid::walk_on_y_boundary(VECTOR3 pp, int plane_pos,double& min_d2,VECTOR3& min_v)
{
	//dir:FEL_I;
...
int	CurvilinearGrid::walk_on_z_boundary(VECTOR3 pp, int plane_pos,double& min_d2,VECTOR3& min_v)
{
	//dir:FEL_I;
...
//			if ((return_iblank && (1 <= res && res <= 15)) || res == 1)
	...
	// fourth try
		...
		VECTOR3 v, p_v, n, n_v;
		double d2, min_d2 = 3.402823466e+38F;
		VECTOR3 min_v;//cell

"""
Plot reassembled pathlines on top of the DrawSubdomain ocean mask.

Reads pathlines.bin (output of read_traces.py) and draws each pathline
as a lat/lon polyline over the ocean background map.

Binary format of pathlines.bin:
  [int32 nPathlines]
  For each pathline:
    [int32 pid][int32 nPts][float64 x0][float64 y0][float64 z0] ...

Usage:
  python plot_pathlines.py [pathlines_bin] [draw_dir] [output_png]
  python plot_pathlines.py trace_outputs/pathlines.bin drawSubdomain pathlines_map.png
"""
...
def read_pathlines_bin(path):
    """
    Read pathlines.bin produced by read_traces.py.
    Returns list of dicts: {'pid': int, 'pts': [(lat, lon, depth_m), ...]}
    depth_m = EARTH_RADIUS - r  (positive = below surface)
    """
    pathlines = []
    with open(path, 'rb') as f:
        n_pl, = struct.unpack('i', f.read(4))
        for _ in range(n_pl):
            ...
            pathlines.append({'pid': pid, 'pts': pts})
    return pathlines
...
def main():
    sl_file  = sys.argv[1] if len(sys.argv) > 1 else 'pathlines/pathlines.bin'
    draw_dir = sys.argv[2] if len(sys.argv) > 2 else 'drawSubdomain'
    out_png  = sys.argv[3] if len(sys.argv) > 3 else 'pathlines_map.png'

    print(f"Ocean mask   : {draw_dir}/{LAT_RES}_{LON_RES}.bin")
    print(f"Pathlines  : {sl_file}")

    ocean_mask  = read_ocean_mask(draw_dir)
    pathlines = read_pathlines_bin(sl_file)
    print(f"Loaded {len(pathlines)} pathlines")
    npts_list = [len(sl['pts']) for sl in pathlines]
    if npts_list:
        print(f"Points per pathline — min:{min(npts_list)}  max:{max(npts_list)}  mean:{sum(npts_list)/len(npts_list):.1f}")
        print(f"Pathlines with >1 pt: {sum(1 for n in npts_list if n > 1)} / {len(npts_list)}")
    ...
    # Global depth range for a consistent colormap across all pathlines
    all_depths = [p[2] for sl in pathlines for p in sl['pts']]
    ...
    # Mark start points
    start_lons = [sl['pts'][0][1] for sl in pathlines]
    start_lats = [sl['pts'][0][0] for sl in pathlines]
    ...
    ax.set_title(f'Pathlines on ocean mask  (n={len(pathlines)})')
